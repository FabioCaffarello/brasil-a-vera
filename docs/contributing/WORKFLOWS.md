...
| `ingestion-monthly.yml` | `schedule`, `workflow_dispatch` | `0 4 1 * *` (dia 1, 04:00 UTC) | Fontes quase-estáticas: TSE (bens, votação municipal), lideranças, blocos, frentes, filiações, backfills de CPF/bio. Config-driven: `discover`→`tier0/1/2` (cadence=monthly, ADR-035). | `ingestion-monthly` (no cancel) |
| `ingestion-retry.yml` | `workflow_run` (Ingestão diária/semanal/mensal, `completed`) | — | Auto-retry-once: quando um run de ingestão termina em `failure` ou `cancelled` na attempt 1, re-executa os jobs falhos/cancelados (runner novo = IP novo — mitigação do drop silencioso de IPs do GH Actions pelo firewall do leg.br, issue #701; job morto por `timeout-minutes` conclui como `cancelled` e também é coberto). Run cancelado sem nenhum job verde = cancelamento deliberado do operador, não é ressuscitado. Anti-loop via `run_attempt == 1`; a falha da attempt 2 gera incidente normalmente (`notify-failure` com `skip-first-attempt`). | — (evento pontual por run) |
