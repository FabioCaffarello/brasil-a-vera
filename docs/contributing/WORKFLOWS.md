...
| `ingestion-retry.yml` | `workflow_run` (Ingestão diária/semanal/mensal, `completed`) | — | Auto-retry-once: quando um run de ingestão termina em `failure` na attempt 1, re-executa os jobs falhos (runner novo = IP novo — mitigação do drop silencioso de IPs do GH Actions pelo firewall do leg.br, issue #701). Anti-loop via `run_attempt == 1`; a falha da attempt 2 gera incidente normalmente (`notify-failure` com `skip-first-attempt`). | — (evento pontual por run) |
