...
const trustExamples: { level: TrustLevel; example: string }[] = [
  ...
  { level: 'L2', example: 'Total de votos a favor por parlamentar (agregação)' },
...
export default function HomePage() {
  return (
    <div className="mx-auto max-w-3xl px-6 py-12 sm:py-16">
      ...
      <section id="piramide-confianca" aria-labelledby="piramide-confianca-titulo">
        <h2
        ...
        <p className="mb-6 text-sm text-zinc-600 dark:text-zinc-400">
          Todo dado exibido no Brasil a Vera carrega um nível de confiança explícito. Nenhum número aparece sem que você saiba de onde veio.
        </p>
        <ul className="space-y-4">
          {trustExamples.map(({ level, example }) => (
            <li key={level} className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
              <div className="mb-2">
                <TrustBadge trustLevel={level} />
              </div>
              <p className="text-sm text-zinc-700 dark:text-zinc-300">{TRUST_LEVEL_DESCRIPTIONS[level]}</p>
              <p className="mt-1 text-sm italic text-zinc-500 dark:text-zinc-400">Ex: {example}</p>
            </li>
          ))}
        </ul>
      </section>
