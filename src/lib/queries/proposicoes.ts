...
export interface ProposicaoKey {
  tipo: TipoProposicao
  numero: number
  ano: number
}
...
/**
 * Lista chaves (tipo, numero, ano) das proposições em tramitação ativa
 * (limit default 500).
 *
 ...
 * produz JSON de chaves commitado no repo, consumido pelo build sem
 ...
 * aplicável.
 */
export async function getProposicaoKeysEmTramitacao(
  limit = 500,
): Promise<ProposicaoKey[]> {
  const rows = await db
    .select({
      tipo: proposicao.tipo,
      numero: proposicao.numero,
    ...
    .from(proposicao)
    .where(eq(proposicao.situacao, 'TRAMITANDO'))
    .orderBy(desc(proposicao.ano), desc(proposicao.numero))
    .limit(limit)
  return rows.map((r) => ({
    tipo: r.tipo as TipoProposicao,
    numero: r.numero,
    ano: r.ano,
  }))
}
