// Rota piloto-2: cópia-rds de /parlamentares/[id] sob /rds/ para validar
// a migração para o @fabio.caffarello/react-design-system 3.7.0 — o
// primeiro dos 3 perfis de detalhe (trio N1+N4+N6 fechado upstream:
// Card compound 3.5.0, useScrollSpy 3.6.0, Stat/StatGroup 3.7.0).
...
// - KpiStrip (composição local)  → StatGroup + Stat do /server
// - SectionCard (composição local) → Card compound do /server (asSection)
// - SectionNav (composição local)  → ./_components/section-nav (client,
//   IntersectionObserver local — useScrollSpy do RDS puxa o barrel client
//   inteiro (+277KB); entry ./hooks pedido upstream em RDS #203)
// - Accordion mobile: primitiva Radix LOCAL mantida — o Accordion do RDS
//   corta conteúdo alto (max-h-[1000px]) e não aceita typography custom
//   no trigger; gap reportado upstream em RDS #202.
//
...
function buildPerfilHref(
  ...
  return `/rds/parlamentares/${parlamentarId}${suffix}`
}

export async function generateMetadata({ params }: PageProps) {
  const { id } = await params
  const parlamentar = await getParlamentarById(id)
  if (!parlamentar) return { title: 'Parlamentar (rds-pilot) — Brasil à Vera' }
  const cargo = parlamentar.casa === 'CAMARA' ? 'Deputado Federal' : 'Senador'
  const title = `${parlamentar.nome} (${parlamentar.partidoSigla}/${parlamentar.uf}) (rds-pilot) — Brasil à Vera`
