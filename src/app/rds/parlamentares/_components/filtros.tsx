// Cópia-rds de src/components/parlamentar/filtros.tsx — onda HeroSection
// (listagem /parlamentares). Original INTOCADO.
//
// Substituições estruturais vs original:
// - FilterChips (wrapper)  → RDS /server (server-safe; §3.9, varredura 3.10.0)
// - FilterChip (item)      → ./filter-chip LOCAL (zero-JS; Chip do RDS é
//                            client +5.759 bytes/rota, chips são <Link>, ADR-022)
// - Label                  → RDS /server (server-safe, <label> nativo;
//                            precedente piloto-6 — token via className traduzido)
// - Combobox               → import do ORIGINAL (client island; cmdk+popover,
//                            lógica preservada — precedente client islands)
// - Button                 → ./button LOCAL traduzido (Button do RDS é client)
//
// hrefs, form `action` e link "Limpar" reescritos para /rds/parlamentares
// (estado de filtro CONTIDO na rota staging — não vaza pro usuário da
// rota original).
//
 ...
 * ativos. Override com `null` remove o filtro do URL. Base /rds/.
 */
function buildHref(
  ...
  return query ? `/rds/parlamentares?${query}` : '/rds/parlamentares'
}
...
/**
 * Filtros de parlamentares (cópia-rds). Mesmo contrato do original:
 * Casa via FilterChips (Links, URL=state); busca por nome (input search,
 * SSR puro); partido + UF via Combobox (client island do original);
 * ordem via select nativo; chips de filtros ativos abaixo.
 */
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          <Button asChild size="sm" variant="outline">
            <a href="/rds/parlamentares">Limpar</a>
          </Button>
