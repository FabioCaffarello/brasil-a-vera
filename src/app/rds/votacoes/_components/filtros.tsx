// Cópia-rds de src/components/votacao/filtros.tsx — onda HeroSection
// (listagem /votacoes). Original INTOCADO.
//
// Substituições estruturais vs original (mesmo padrão das listagens
// /parlamentares §3.14 e /proposicoes §3.15):
...
//
// Sem Combobox (Ano é `<select>` — cardinalidade média, diferente do Tema
// das proposições). Sem busca livre `q` (a listagem de votações não
// indexa texto). hrefs, form `action` e link "Limpar" reescritos para
// /rds/votacoes (estado de filtro CONTIDO na rota staging).
//
...
function buildHref(
  ...
  return query ? `/rds/votacoes?${query}` : '/rds/votacoes'
}
