// Piloto-5: cópia-rds de /privacidade sob /rds/ (strangler fig, ADR-033).
// Convive em paralelo com a rota original; promoção é decisão futura.
//
// Rota textual sem componentes de domínio em src/components/ — os helpers
// locais (Section, ContactLink) são reconstruídos AQUI, como na piloto-1
// (helper local não vira arquivo separado). Espelhamento é em nível de
// página: par registrado em docs/migration/consolidation-debt.md.
//
...
// `docs/migration/token-map.md` (cor via classe traduzida; <Text> só
// quando o variant cobre a typography com ≤1 override).
//
// Versão do texto vive em `src/lib/privacy.ts` (PRIVACY_POLICY_VERSION).
// O link "texto-fonte em GitHub" continua apontando pro arquivo da rota
// ORIGINAL — ela é a fonte canônica do texto enquanto /rds/ for staging.
...
// Mesma justificativa da rota original: `auth()` no <Navbar /> via
// <AuthSlot /> exige clerkMiddleware em runtime; prerender no build falha.
export const dynamic = 'force-dynamic'

export const metadata: Metadata = {
  title: 'Política de Privacidade (rds-pilot) — Brasil à Vera',
  description:
